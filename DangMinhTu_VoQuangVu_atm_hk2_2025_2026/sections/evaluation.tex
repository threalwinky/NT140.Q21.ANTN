...
Toàn bộ thực nghiệm chính được chạy trên pipeline \texttt{reproduction/src/run\_full\_demo.py} và các script liên quan. Bộ dữ liệu được tách train/test theo tỉ lệ 80/20, sau đó ba mô hình DT, RF và DT-CTS được huấn luyện trên cùng một tập train để bảo đảm so sánh công bằng. Việc đánh giá được tiến hành theo hai hướng:
\begin{itemize}
...
Kết quả của luồng tái hiện chính cho thấy:
\begin{itemize}
...
Nếu chỉ xét riêng F1 thì DT là mô hình đạt kết quả tốt nhất trong lần chạy hiện tại. Tuy nhiên, với mục tiêu của SISTAR, điểm quan trọng hơn là DT-CTS vẫn duy trì chất lượng ở mức cạnh tranh trong khi được thiết kế để giảm số threshold. Đây là yếu tố có ý nghĩa rõ ràng khi xét tới khả năng triển khai trên programmable switch, nơi chi phí bảng match và số điều kiện so sánh không thể bỏ qua.

\begin{figure}
\centering
\includegraphics[width=0.78\textwidth]{classification_metric_suite.png}
...
\midrule
DT & 36 & Baseline dễ diễn giải nhưng vẫn tương đối nặng \\
RF & lớn & Khó phù hợp với data plane \\
DT-CTS & 14 & Giảm 61.11\% so với DT, phù hợp hơn cho switch \\
\bottomrule
\end{tabular}
\end{table}

Trong luồng tái hiện chính, DT sử dụng 36 threshold trong khi DT-CTS chỉ sử dụng 14 threshold, tương đương mức giảm khoảng 61.11\%. Ở benchmark paper-style với 3, 5 và 7 feature, cấu hình DT-CTS tốt nhất đạt F1 0.9614 với 14 threshold, đồng thời vẫn giảm 58.82\% số threshold so với DT cùng cấu hình. Kết quả này cho thấy DT-CTS không nhất thiết vượt trội tuyệt đối về chất lượng phân loại, nhưng thể hiện ưu thế rõ ràng hơn ở trade-off giữa chất lượng và khả năng triển khai.

\begin{figure}
\centering
\begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{threshold_usage.png}
    \caption{Threshold usage trong reproduction chính.}
    \label{fig:threshold-usage}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{benchmark_f1_scores.png}
...
Trong phần giảm thiểu, nghiên cứu so sánh ba policy:
\begin{enumerate}
...
\item \texttt{hierarchical\_confidence\_pushback}: policy cải tiến được đề xuất trong nghiên cứu này.
\end{enumerate}
...
So với baseline không pushback, policy mới làm giảm khoảng 99.92\% attack bytes. Đồng thời, nó vẫn giữ được 2{,}052{,}047{,}683 benign bytes tới victim, cao hơn rõ rệt so với immediate pushback. Kết quả này cho thấy phần cải tiến đề xuất có giá trị thực tiễn: thay vì triệt tiêu attack bằng mọi giá, cơ chế mới ưu tiên một điểm cân bằng tốt hơn giữa phòng thủ và tính sẵn sàng của traffic hợp lệ.

\begin{figure}
\centering
\includegraphics[width=0.93\textwidth]{pushback_clean_summary.png}
\caption{Tóm tắt trực quan kết quả của ba policy pushback. Hình chỉ giữ lại các thông tin quan trọng nhất: lượng attack traffic còn tới victim, lượng benign traffic được giữ lại, số lần false block và số lần leo thang ở policy cải tiến.}
...
Từ các kết quả trên có thể rút ra ba nhận xét. Thứ nhất, DT-CTS là một lựa chọn hợp lý khi mục tiêu bao gồm cả khả năng triển khai trong data plane, thay vì chỉ tối ưu chất lượng phân loại. Thứ hai, immediate pushback rất hiệu quả trong việc triệt tiêu attack nhưng lại quá nhạy với false positive. Thứ ba, hierarchical confidence-aware pushback xử lý bài toán ở cấp hệ thống tốt hơn vì nó cho phép phản ứng theo mức, gần với cách một hệ thống thực tế nên vận hành.

Nghiên cứu vẫn còn một số giới hạn. Các kết quả pushback hiện mới được đo trong mô phỏng flow-level, chưa phải trên mạng thật hoặc ASIC thật. Ngoài ra, threshold count cũng chỉ là chỉ báo gián tiếp cho footprint phần cứng. Dù vậy, các kết quả hiện tại vẫn đủ để cho thấy hướng cải tiến đề xuất là hợp lý và có ý nghĩa hơn một thay đổi nhỏ ở ngưỡng block.
