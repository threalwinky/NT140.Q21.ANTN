...
def synthesize_flows(n_benign: int = 2400, n_attack: int = 2400, seed: int = 42) -> pd.DataFrame:
    ...
    benign_ports = rng.choice([22, 53, 80, 123, 443, 8080], size=n_benign, p=[0.05, 0.1, 0.25, 0.05, 0.45, 0.1])
    ...
    attack_ports = np.empty(n_attack)
    ...
    for i, kind in enumerate(attack_kind):
        if kind == "syn":
            attack_ports[i] = rng.choice([80, 443, 8080], p=[0.55, 0.35, 0.10])
            ...
            attack_ports[i] = rng.choice([53, 123, 443], p=[0.40, 0.35, 0.25])
            ...
            attack_ports[i] = rng.choice([80, 443], p=[0.6, 0.4])
    ...
    benign = pd.DataFrame(
        {
            "destination_port": benign_ports.astype(int),
    ...
    attack = pd.DataFrame(
        {
            "destination_port": attack_ports.astype(int),
...
def load_real_wednesday_subset(per_class: int = 2500, seed: int = 42) -> pd.DataFrame:
    usecols = list(REAL_TO_CANONICAL.keys())
    df = pd.read_csv(REAL_WEDNESDAY_CSV, usecols=usecols, low_memory=False)
    df = df.rename(columns=REAL_TO_CANONICAL)
    ...
    df["label"] = (df["raw_label"] != "BENIGN").astype(int)
